...
export class AuthController {
  async login(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      logger.debug(`[AuthController] Login attempt for sub=${auth0User.sub} email=${auth0User.email}`);
      ...
      if (!user) {
        logger.warn(`[AuthController] Usuario no registrado en la plataforma: ${auth0User.email}`);
      ...
      if (!user.enabled || !user.sub) {
        logger.warn(`[AuthController] Usuario deshabilitado o sin sub: ${user.email} enabled=${user.enabled} sub=${user.sub}`);
      ...
      logger.debug(`[AuthController] Login exitoso para ${user.email}, roles=[${roles.join(', ')}]`);
      ...
      if (!user.fullName) {
        ...
        if (name) {
          logger.info(`[AuthController] Fetcheando nombre para ${user.email} desde Auth0`);
      ...
      if (error.statusCode === 403) {
        logger.warn(`[AuthController] Acceso denegado (403): ${error.message}`);
        res.status(403).json(formatErrorResponse(error.message || 'Usuario no habilitado para acceder a la plataforma', 403));
        return;
      }
      logger.error('[AuthController] Error inesperado en login:', error);
