...
import { Upload, FileText, Users, CheckCircle, XCircle, AlertCircle, Loader2 } from 'lucide-react';
...
export default function Admin() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        // Reload user status after successful upload
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (file) {
      if (!file.name.toLowerCase().endsWith('.csv')) {
        setError('Por favor, arrastra un archivo CSV');
        return;
      }
      setSelectedFile(file);
      setError(null);
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    Sube un archivo CSV para habilitar/deshabilitar usuarios
                  </p>
                </div>
              </div>
              ...
              {uploadResult && (
                <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-lg">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
                    <div>
                      <p className="text-xs text-green-700 mb-1">Habilitados</p>
                      <p className="text-lg font-semibold text-green-900">
                        {uploadResult.enabled}
                      </p>
                    </div>
                    <div>
                      <p className="text-xs text-green-700 mb-1">Deshabilitados</p>
                      <p className="text-lg font-semibold text-green-900">
                        {uploadResult.disabled}
                      </p>
                    </div>
                    <div>
                      <p className="text-xs text-green-700 mb-1">Creados</p>
                      <p className="text-lg font-semibold text-green-900">
              ...
              {isLoadingStatus ? (
                ...
                <div className="space-y-4">
                    ...
                    <div className="mt-2">
                      <div className="w-full bg-green-200 rounded-full h-2">
                        <div
                          ...
                          style={{
                            width: `${(userStatus.enabled / userStatus.total) * 100}%`,
                          }}
                        />
                      </div>
                    </div>
                    ...
                    <div className="mt-2">
                      <div className="w-full bg-red-200 rounded-full h-2">
                        <div
                          ...
                          style={{
                            width: `${(userStatus.disabled / userStatus.total) * 100}%`,
                          }}
                        />
                      </div>
                    </div>
                  ...
                  <button
                    ...
                    className="w-full mt-4 py-2 px-4 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg text-sm font-medium transition-colors"
                  >
                    Actualizar
                  </button>
                </div>
